...
namespace ManagementConsoleBackend.ManagementService
{
    public class EventHandlers
    {
        ...
        private async Task<bool> HandleSimulatorFlexMatchEvent(FlexMatchEvent flexMatchEvent)
        {
            ...
            /*
            var dynamoDbRequestHandler = new DynamoDbRequestHandler(dynamoDbClient);
            var latestSimulation =
                await dynamoDbRequestHandler.GetMatchmakingSimulation(flexMatchEvent.Detail.CustomEventData);

            var currentTimestamp = Utils.GetUnixTimestamp();
            if (currentTimestamp > latestSimulation.LastUpdateSent)
            {
                var timeUpdateRequest = new UpdateItemRequest
                {
                    TableName = Environment.GetEnvironmentVariable("MatchmakingSimulationTableName"),
                    Key = new Dictionary<string, AttributeValue>()
                        {{"SimulationId", new AttributeValue {S = flexMatchEvent.Detail.CustomEventData}}},
                    UpdateExpression = "SET #lastUpdateSent = :currentTimestamp",
                    ExpressionAttributeNames = new Dictionary<string, string>
                    {
                    ...
                    ExpressionAttributeValues = new Dictionary<string, AttributeValue>()
                    {
                        {":currentTimestamp", new AttributeValue {N = currentTimestamp.ToString()}},
                    }
                };
                
                try
                {
                    await dynamoDbClient.UpdateItemAsync(timeUpdateRequest);
                }
                catch (Exception e)
                {
                    LambdaLogger.Log(e.Message);
                ...
                await ManagementService.SendToActiveConnections(new ServerMessageMatchmakingSimulationUpdate
                {
